  ...
  methods: {
    ...
    logSelectedRows() {
      ...
      if (selectedRows.length > 0) {
        const poName = selectedRows[0].PO_NAME;
        // push：會將新的路由添加到歷史紀錄堆疊中。用戶可以按瀏覽器的“上一頁”按鈕返回到之前的路由。
        // replace：會用新的路由替換當前路由，並不會在歷史紀錄堆疊中添加新的條目。這會覆蓋當前路由，無法通過“上一頁”按鈕返回到之前的路由。
        this.$router.replace({ path: '/title', query: { poName: poName } });
      } else {
        alert('請選擇至少一筆');
      }
    },
